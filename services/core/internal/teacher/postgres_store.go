...
// This mirrors learner's licensed-source eligibility. A snapshot cannot be created when a
// licensed adaptation's provenance source has regressed its approval, rights, or catalogue link.
const assignmentQuestion = `SELECT q.id,q.response_type,q.language,q.prompt,q.options,q.content_revision FROM questions q JOIN curriculum_map_nodes n ON n.id=q.curriculum_map_node_id JOIN content_sources s ON s.id=n.content_source_id JOIN question_rubric_versions rv ON rv.id=q.canonical_rubric_version_id AND rv.question_id=q.id LEFT JOIN question_provenance qp ON qp.question_id=q.id AND qp.origin_type=q.origin_type LEFT JOIN content_sources ps ON ps.id=qp.content_source_id WHERE q.id=$1 AND q.syllabus_id=$2 AND q.curriculum_map_node_id=$3 AND q.status='verified' AND rv.status='verified' AND rv.question_revision=q.content_revision AND n.status='verified' AND s.status='approved' AND s.catalogue_syllabus_id=q.syllabus_id AND (q.origin_type IS NULL OR q.origin_type='original' OR (q.origin_type='licensed_adaptation' AND qp.question_id IS NOT NULL AND btrim(qp.source_locator)<>'' AND ps.status='approved' AND ps.catalogue_syllabus_id=q.syllabus_id AND NULLIF(btrim(ps.owner),'') IS NOT NULL AND NULLIF(btrim(ps.title),'') IS NOT NULL AND NULLIF(btrim(ps.source_url),'') IS NOT NULL AND NULLIF(btrim(ps.source_hash),'') IS NOT NULL AND NULLIF(btrim(ps.licence_reference),'') IS NOT NULL AND NULLIF(btrim(ps.permitted_use),'') IS NOT NULL AND NULLIF(btrim(ps.allowed_audience),'') IS NOT NULL))`
