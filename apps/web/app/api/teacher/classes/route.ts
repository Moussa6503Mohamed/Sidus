import {NextResponse} from "next/server";import{callTeacherCore,TeacherProxyError}from "@/lib/teacher/core-proxy";
...
export async function POST(r:Request){try{const b=await r.text();const x=await callTeacherCore({kind:"createClass",body:b});return NextResponse.json(x.body,{status:x.status})}catch(e){return error(e)}}
