...
describe("POST /api/editorial/curriculum-map/nodes/[id]/retire", () => {
  it("ignores any client-supplied body and sends a fixed empty object to Core", async () => {
    mockedCallCore.mockResolvedValue({ status: 200, body: { id: "node-1", status: "retired" } });
    ...
    expect(mockedCallCore).toHaveBeenCalledWith({ kind: "retireCurriculumMapNode", id: "node-1" }, "{}");
