...
describe("POST /api/editorial/content-sources/[id]/approve", () => {
  it("ignores any client-supplied body and sends a fixed empty object to Core", async () => {
    mockedCallCore.mockResolvedValue({ status: 200, body: { status: "approved" } });

    const response = await POST(
      new Request("http://localhost/x", {
        method: "POST",
        body: JSON.stringify({ reviewerId: "attacker-supplied" }),
      }),
      { params: Promise.resolve({ id: "abc-1" }) },
    );

    expect(mockedCallCore).toHaveBeenCalledWith({ kind: "approveContentSource", id: "abc-1" }, "{}");
