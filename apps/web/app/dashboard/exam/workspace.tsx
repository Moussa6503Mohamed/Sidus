...
export function ExamWorkspace({ durationSeconds = DURATION_SECONDS }: { durationSeconds?: number }) {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.card}><p className={styles.muted}>Question {index + 1} of {questions.length}</p><p id={`exam-question-${question.id}`} className={styles.prompt}>{question.prompt}</p><AnswerControl question={question} answer={answers[question.id]} disabled={submitting} setAnswer={setAnswer} /></section>
