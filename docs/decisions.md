...
**Update (T-0024):** The private dev CA generated by the original `openssl req -x509 -new
-nodes ...` one-liner had no `-config`/`-extensions`, so it carried `basicConstraints =
critical, CA:TRUE` (openssl's own default for a self-signed `req -x509` cert) but **no
`keyUsage` extension at all**. `ssl.create_default_context(cafile=ca.pem)` — the exact call the
T-0022 client's `SIDUS_CORE_CA_BUNDLE` path uses — rejects a CA cert missing `keyUsage` with
`CERTIFICATE_VERIFY_FAILED: CA cert does not include key usage extension`; reproduced against
the regenerated stack before the fix, confirming root cause. The leaf cert's config also lacked
`basicConstraints = CA:false` and an `authorityKeyIdentifier`. Fix: the CA is now generated from
an explicit `ca.cnf` (`x509_extensions = v3_ca`: `basicConstraints = critical, CA:true`,
`keyUsage = critical, keyCertSign, cRLSign`, `subjectKeyIdentifier = hash`); the leaf CSR keeps
requesting only `subjectAltName` (an `authorityKeyIdentifier` cannot be computed before a CSR
has an issuer), and the full leaf extension set — `basicConstraints = critical, CA:false`,
`keyUsage = critical, digitalSignature, keyEncipherment`, `extendedKeyUsage = serverAuth`,
`subjectAltName` (`DNS:localhost`, `IP:127.0.0.1`), `authorityKeyIdentifier = keyid,issuer` — is
applied at the `x509 -req -CA ... -extensions v3_leaf` signing step, where the CA is available
to derive the identifier from. `docs/local-import-test-environment.md` updated to match; no
Compose/Caddy/Core/client code changed — this was a certificate-generation defect only. See
